...
uint8_t *g_ai_buf = (uint8_t *) 0x40600000;
...
static float layer0_anchor[ANCHOR_NUM * 2]= {
                                             0.77730761, 0.81060384,
                                             0.5835267,  0.39886141,
                                             0.40827086, 0.75383102
//    0.76120044, 0.57155991, 0.6923348, 0.88535553, 0.47163042, 0.34163313,
};

static float layer1_anchor[ANCHOR_NUM * 2]= {
                                             0.22321318, 0.56973644,
                                             0.18510336, 0.28125034,
                                             0.07241924, 0.14491876
...
static void print_all_boxes(region_layer_t *rl) {
  for (int i = 0; i < rl->boxes_number; i++) {
    int j;
    for (j = 0; j < rl->classes; j++) {
      if (rl->probs[i][j] > 0) break;
    }
    if (j < rl->classes) {
      printf("[%d/%d]", i, rl->boxes_number);
      for (int j = 0; j < rl->classes; j++) {
        printf(" %.2f", rl->probs[i][j]);
      }
      printf("\r\n");
    }
  }
...
int main(void) {
    ...
    lcd_draw_string(104, 150, "YOLOv3 20 class detection", WHITE);
    ...
    while (1)
    {
        ...
        /* display result */
        int drawn = region_layer_draw_boxes(&detect_rl0, drawboxes);
        printf("RL0\r\n");
        if (drawn > 0) {
          print_all_boxes(&detect_rl0);
        }

        drawn = region_layer_draw_boxes(&detect_rl1, drawboxes);
        printf("RL1\r\n");
        if (drawn > 0) {
          print_all_boxes(&detect_rl1);
        }
        /* sleep(10); */
    }
